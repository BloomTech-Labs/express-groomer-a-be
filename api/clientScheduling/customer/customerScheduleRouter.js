...
router.get('/', async (req, res) => {
    try {
        ...
        const data = await schedule.findAppointmentsByCustomerId(customer_id);
...
router.get('/:groomer_id', async (req, res) => {
    try {
        ...
        const valid = await schedule.validGroomer(groom_id);

        if(!valid.length){
            return res.status(400).json({ message: "Provided groomer does NOT exist" });
        }
